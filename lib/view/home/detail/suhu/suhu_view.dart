...
class _SuhuViewState extends State<SuhuView> {
  ...
  void onLogic(BuildContext context,
    ...
    if (inputValue == '°C' && inputResult == '°C') {
      context
          .read<SuhuBloc>()
          .add(CelsiusToCelsius(c: double.tryParse(inputController.text) ?? 0));
      print('c - c');
    }
    if (inputValue == '°C' && inputResult == '°F') {
      context.read<SuhuBloc>().add(
          CelsiusToFahrenheit(c: double.tryParse(inputController.text) ?? 0));
      print('c - f');
    }
    if (inputValue == '°C' && inputResult == 'Kelvin') {
      context
          .read<SuhuBloc>()
          .add(CelsiusToKelvin(c: double.tryParse(inputController.text) ?? 0));
      print('c - Kelvin');
    }
    //
    if (inputValue == '°F' && inputResult == '°C') {
      context.read<SuhuBloc>().add(
          FahrenheitToCelsius(f: double.tryParse(inputController.text) ?? 0));
      print('f - c');
    }
    if (inputValue == '°F' && inputResult == '°F') {
      context.read<SuhuBloc>().add(FahrenheitToFahrenheit(
          f: double.tryParse(inputController.text) ?? 0));
      print('f - f');
    }
    if (inputValue == '°F' && inputResult == 'Kelvin') {
      context.read<SuhuBloc>().add(
          FahrenheitToKelvin(f: double.tryParse(inputController.text) ?? 0));
      print('f - Kelvin');
    }
    //
    if (inputValue == 'Kelvin' && inputResult == '°C') {
      context
          .read<SuhuBloc>()
          .add(KelvinToCelsius(k: double.tryParse(inputController.text) ?? 0));
      print('kelvin - c');
    }
    if (inputValue == 'Kelvin' && inputResult == '°F') {
      context.read<SuhuBloc>().add(
          KelvinToFahrenheit(k: double.tryParse(inputController.text) ?? 0));
      print('kelvin - f');
    }
    if (inputValue == 'Kelvin' && inputResult == 'Kelvin') {
      context
          .read<SuhuBloc>()
          .add(KelvinToKelvin(k: double.tryParse(inputController.text) ?? 0));
      print('kelvin - Kelvin');
    }
  }
  ...
  void showFormula(
    ...
    if (inputValue == '°C' && inputResult == '°F') {
      context
          .read<SuhuBloc>()
          .add(ShowFormula(formula: '$initial (°C × 1.8) + 32'));
    }
    if (inputValue == '°C' && inputResult == 'Kelvin') {
      context
          .read<SuhuBloc>()
          .add(ShowFormula(formula: '$initial °C + 273,15'));
    }
    //
    if (inputValue == '°F' && inputResult == '°C') {
      context
          .read<SuhuBloc>()
          .add(ShowFormula(formula: '$initial (°F − 32) × 0.556'));
    }
    if (inputValue == '°F' && inputResult == '°F') {
      context.read<SuhuBloc>().add(ShowFormula(formula: ''));
    }
    if (inputValue == '°F' && inputResult == 'Kelvin') {
      context
          .read<SuhuBloc>()
          .add(ShowFormula(formula: '$initial (°F − 32) × 1.8 + 273,15 '));
    }
    //
    if (inputValue == 'Kelvin' && inputResult == '°C') {
      context.read<SuhuBloc>().add(ShowFormula(formula: '$initial K − 273,15'));
    }
    if (inputValue == 'Kelvin' && inputResult == '°F') {
      context
          .read<SuhuBloc>()
          .add(ShowFormula(formula: '$initial (K − 273,15) × 1.8 + 32'));
    }
    if (inputValue == 'Kelvin' && inputResult == 'Kelvin') {
      context.read<SuhuBloc>().add(ShowFormula(formula: ''));
    }
